...
test("platform chrome overwrites share-card metas and always sets og:title", () => {
  ...
  const out = injectGrokPwaHead(html, { appName: "Wild Race", cwd: mkdtempSync(join(tmpdir(), "grok-og-empty-")) });
...
test("published grok.me slug is still a title fallback", () => {
  const out = injectGrokPwaHead("<html><head></head></html>", {
    host: "wild-race.grok.me",
    cwd: mkdtempSync(join(tmpdir(), "grok-og-empty-")),
  });
...
test("injects into documents with no head element", () => {
  const out = injectGrokPwaHead("<html><body>hi</body></html>", { appName: "Solo", cwd: mkdtempSync(join(tmpdir(), "grok-og-empty-")) });
...
test("streaming injector matches </HEAD> case-insensitively", () => {
  const injector = createHeadInjector({ appName: "Wild Race", cwd: mkdtempSync(join(tmpdir(), "grok-og-empty-")) });
  const chunks = [
...
test("uses the app name in the injected title tag", () => {
  const out = injectGrokPwaHead("<html><head></head></html>", { appName: "Wild Race", cwd: mkdtempSync(join(tmpdir(), "grok-og-empty-")) });
